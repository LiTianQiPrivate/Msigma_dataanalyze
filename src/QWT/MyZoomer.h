...
public:
    ...
    virtual  QwtText trackerTextF(const QPointF &pos )const
    {
        ...
        if(point.size() > 1)
        {
            ...
            {
                text += "\r\n";
                text += "x" + QString::number(i)+":" +QString("%1").arg(point[i].x(),0, 'f', 1)+" " +"y" + QString::number(i) + ":" + QString( "%1" ).arg( point[i].y(), 0, 'f', 1 );
            }
        }
        else
        {
            text = "x:" + QString("%1").arg(point[0].x(),0, 'f', 1)+" " + "y:" + QString( "%1" ).arg( point[0].y(), 0, 'f', 1 );
        }
