...
CurvePlot::CurvePlot(QWidget *parent, QString className):QwtPlotBase(parent,className)
{
    ...
    connect(picker,SIGNAL(zoomer(const QRectF&)),this,SLOT(on_zoomer(const QRectF&)));
}
CurvePlot::~CurvePlot()
{
...
void CurvePlot::on_zoomer(const QRectF &rect)
{
...
void CurvePlot::setModelBases(std::vector<ModelBase *> modelObjects, int index)
{
    ...
    for(int i = 0; i < modelObjects.size(); i++)
    {
        ...
        setAxisScale(QwtPlot::yLeft,yMax, yMin,10);
        setAxisScale(QwtPlot::xBottom,xMin,xMax,30);
        setAxisScale(QwtPlot::yRight,yMax, yMin,10);
        setAxisScale(QwtPlot::xTop,xMin,xMax,30);
...
void CurvePlot::on_checkTurnBoxPointClick(bool v)
{
    ...
    if(v)
    {
        ...
        setAxisScale(QwtPlot::yLeft,xMin,xMax,30);
        setAxisScale(QwtPlot::xBottom,yMin,yMax,10);
        setAxisScale(QwtPlot::yRight,xMin,xMax,30);
        setAxisScale(QwtPlot::xTop,yMin,yMax,10);
        this->replot();
        setAutoReplot( true );//设置自动重画，相当于更新
